...
extension String {
    ...
    static func dateString() -> String {
        return NSDate.init().string(withFormat: "yyyy-MM-dd HH:mm:ss")!;
    }
    
    ///  获取当前的日期, 格式自定义
    func dateString(format: String) -> String {
        return NSDate.init().string(withFormat: format)!;
    }
    ...
    func toDateStrISO8601(formate: String?) -> String {
        ...
        let dateStr = date?.string(custom: format) ?? ""
        return dateStr
    }
...
//        return rect.size;
        ...
        //            let paragraphStyle = NSMutableParagraphStyle.init();
        //            paragraphStyle.lineBreakMode = lineBreakMode;
        //            attr[NSParagraphStyleAttributeName: fon] = paragraphStyle;
        //        }
        let rect = (self as NSString).boundingRect(with: size, options: [NSStringDrawingOptions.usesLineFragmentOrigin, NSStringDrawingOptions.usesFontLeading], attributes: attributes, context: nil);
        return rect.size;
